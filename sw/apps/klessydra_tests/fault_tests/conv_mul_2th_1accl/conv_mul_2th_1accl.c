...
int main(){
  ...
  __asm__("csrrw zero, mcycle, zero");
	...
	if (th_id == 0) {
		...
		kbcastld((void*)((int*)spmaddrB), (void*)kernels, dimension_B);
		kbcastld((void*)((int*)spmaddrA), (void*)image, dimension_A);
	} else if(th_id == 1){
		CSR_MVTYPE(0x00000002);
 		n=N_ROW_1;
 		m=N_COL_1;
	...
	if(th_id == 0){
		...
		int offset = (SPM_MAX*SPM_MAX)/2;

		int *addrA = (int *)spmaddrB + offset;
		int *addrB = (int *)spmaddrC + offset;
		int *addrC = (int *)spmaddrD + offset;
	...
	/*
		kmemld((void *)(addrA), &m1[0][0],  SIZE_OF_INT*(m*n));
		...
		for(int i=0;i<n;i++){
			for(int j=0;j<u;j++){
				...
				if(i == 0){
					kmemld((void *)((int *)addrB + j*m), &m2_trans[j][0],  SIZE_OF_INT*(m));
				}
				kdotp((void *)((int *)addrC + i*u + j),(void *)((int *)addrB + j*m),(void *)((int *) addrA + i*m));
			}
		}
		kmemstr(&m_out32[0][0],  (void *)((int *)addrC),  u*n*SIZE_OF_INT);
	*/
	}
