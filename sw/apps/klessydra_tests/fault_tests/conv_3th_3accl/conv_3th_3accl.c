...
int output_compare_s0[3][NUM_KERNELS][A_ORDER*A_ORDER]={0};
...
//	  }
		for(int th=0; th<3; th++){
			for(int i=0; i<NUM_KERNELS; i++){
				for(int j=0; j<A_ORDER*A_ORDER; j++){
					output_compare_s0[th][i][j]=777;
				}
			}
		}
#if CHECK == 1
	...
	for(int i=0; i<NUM_KERNELS; i++){
		...
		kmemstr((void*)((int*)output_compare_s0 + th_id*NUM_KERNELS*A_ORDER*A_ORDER + i*A_ORDER*A_ORDER 				),
		 				(void*)((int*)spmaddrC ),
