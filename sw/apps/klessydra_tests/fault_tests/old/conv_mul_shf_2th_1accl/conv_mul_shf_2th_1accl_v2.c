...
unsigned int shft_out0[12];
unsigned int shft_out1[12];
unsigned int relu_data[2] = {0x7FFFFFFF, 0x7FFFFFFF};
unsigned int relu_out[2];
...
unsigned int cmp_out0[4];
unsigned int cmp_out1[4];

// add after GL simulations
unsigned int shf_out32[2][N_ROW_1/2][N_COL_2/2];
unsigned int shf_out16[2][N_ROW_1/2][N_COL_2/2];
unsigned int shf_out8 [2][N_ROW_1/2][N_COL_2/2];
unsigned int m_out8[N_ROW_1/2][N_COL_2/2];
int idx_i, idx_j;
int v_size;
...
int main(){
  ...
  __asm__("csrrw zero, mcycle, zero");
		...
		int offset = 8*A_ORDER*A_ORDER;

		int *addrA = (int *)spmaddrC + offset;
		int *addrB = (int *)spmaddrA + offset;
		int *addrC = (int *)spmaddrB + offset;
...
//		}
	...
	if(th_id == 0){
		...
		for(int i=0; i<NUM_KERNELS; i++){
			...
			//kmemstr((void*)((int*)output_compare_s0 + i*A_ORDER*A_ORDER 				),
			// 				(void*)((int*)spmaddrC + i*A_ORDER*A_ORDER),
			//				SIZE_OF_INT*(	A_ORDER*A_ORDER));
		}
	} else if(th_id == 1){
		for (int i = 0; i < n; i++){
			...
			kmemstr(&m_out32[i][0], (void *)((int *)addrB), u * SIZE_OF_INT);
			kbcast((void *)((int *)addrB), (void *)azzero);
		}
...
shift_pre = 0;
	...
	if(th_id == 1) {
		int d_size = u*m/4;
		v_size = d_size*SIZE_OF_INT;
		// -------- Test another structures -----------
		// ----------- Subtrac ------------

		kmemld((void *)((int *)addrB), &m1[0][0], SIZE_OF_INT * u/2 * m);
		kmemld((void *)((int *)addrA), &m2[0][0], SIZE_OF_INT * u/2 * m);
		
		CSR_MVTYPE(0x00000000);
		for(int i = 0;i < u/2;i++){
			ksubv((void *)((int *)addrC + i*m), (void *)((int *)addrA + i*m), (void *)((int *)addrB + i*m));
		}

		kmemstr((void *)((int *)sub_out), (void *)((int *)addrC), SIZE_OF_INT * u/2 * m);
...
//		}
		...
		kmemstr((void *)((int *)relu_out), (void *)((int *)addrC), SIZE_OF_INT * 2);
		...
		kmemstr((void *)((int *)cmp_out0), (void *)((int *)addrC), SIZE_OF_INT * 4);

		kvslt((void *)((int *)addrC), (void *)((int *)addrB), (void *)((int *)addrA));
		kmemstr((void *)((int *)cmp_out1), (void *)((int *)addrC), SIZE_OF_INT * 4);
