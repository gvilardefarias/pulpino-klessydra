...
int main(){
	...
	int m_out32[3][N_ROW_1][N_COL_2];
	int m_out16[3][N_ROW_1][N_COL_2];
	int  m_out8[3][N_ROW_1][N_COL_2];
	int zero=0;
	...
	if(Klessydra_get_coreID()==0){
	    ...
	    th_id = Klessydra_get_coreID();
		offset = th_id * m * u;

		kmemld((void *)((int *)spmaddrA + offset), &m3[0][0],  SIZE_OF_INT*(m*u));
	}
	...
	if(Klessydra_get_coreID()==0){
		...
		for(int i=0;i<m;i++){
			for(int j=0;j<u;j++){
				...
				if(i == 0){
					kmemld((void *)((int *)spmaddrB + offset + j*m), &m4[j][0],  SIZE_OF_INT*(m));
				}
				kdotp((void *)((int *)spmaddrC + offset + i*u + j),(void *)((int *)spmaddrB + offset + j*m),(void *)((int *)spmaddrA + offset + i*m));
			}
		}
		kmemstr(&m_out32[th_id][0][0],  (void *)((int *)spmaddrC + offset),  u*m*SIZE_OF_INT);
	}
	//finish_count();
	sync_barrier_reset();
	sync_barrier_thread_registration();
	...
	if(Klessydra_get_coreID()==0){
		...
		kmemstr(&m_out16[th_id][0][0],  (void *)((int *)spmaddrC),  u*m*SIZE_OF_INT);
	}
