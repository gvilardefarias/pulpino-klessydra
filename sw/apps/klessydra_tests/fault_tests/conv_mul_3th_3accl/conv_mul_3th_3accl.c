...
int main(){
  ...
  __asm__("csrrw zero, mcycle, zero");
	...
	//		output_compare_s0[th_id][k][i]=777;
	//  	}
	//}
    ...
    if(th_id == 2) {
		for (int i = 0; i < A_ORDER; i++)
		{
			...
			{
				mat_second_A[i][j] = image[i * A_ORDER + j];
			}
		}
	}
#endif
...
/*
    if(th_id == 0) {
		for(int i=0; i<NUM_KERNELS; i++){
			convolution2D_Scaling(A_ORDER, mat_second_A,(int*)kernels + i*B_ORDER*B_ORDER, (int*)output_compare0 + i*A_ORDER*A_ORDER);
			//convolution2D_Scaling(A_ORDER, mat_second_A,(int*)matB + i*B_ORDER*B_ORDER, (int*)output_compare0 + i*A_ORDER*A_ORDER);
...
//		}
	...
	if (th_id == 0){
		...
		for (int i = 0; i < TH_NUM; i++){
			printf("Num_cycles %d: %d\n", i, perf_results[i][0]);
			printf("Num_instr %d: %d\n", i, perf_results[i][1]);
			mean += perf_results[i][0];
		}
