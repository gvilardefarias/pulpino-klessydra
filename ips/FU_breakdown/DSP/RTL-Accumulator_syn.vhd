  ...
  ACCUM_SIMD_1 : if SIMD=1 generate
    ...
    begin
      if rst_ni = '0' then
        ...
        dsp_out_accum_results(((f+1)*(32))-1 downto 32*f) <= (others => '0');
        for g in 0 to (ACCL_NUM - FU_NUM) loop
          ...
          case state_DSP(((h+1)*(2))-1 downto 2*h) is	
            when dsp_exec =>
              if (decoded_instruction_DSP_lat((h)*(DSP_UNIT_INSTR_SET_SIZE) + KDOTP_bit_position)   = '1'  or -- acccumulate 32-bit types
                ...
                if (accum_stage_2_en(h) = '1' or recover_state_wires(h) = '1') and halt_dsp_lat(h) = '0' then
                  dsp_out_accum_results(((f+1)*(32))-1 downto 32*f) <= std_logic_vector(unsigned(accum_partial_results_stg_1(31  + (f)*(128) downto  0 + (f)*(128))) +
                                                            unsigned(dsp_out_accum_results(((f+1)*(32))-1 downto 32*f)));
                end if;
                ...
                if (accum_stage_2_en(h) = '1' or recover_state_wires(h) = '1') and halt_dsp_lat(h) = '0' then
                  dsp_out_accum_results(((f+1)*(32))-1 downto 32*f) <= std_logic_vector(unsigned(accum_partial_results_stg_1(15  + (f)*(128) downto  0 + (f)*(128)))  + 
                                                            unsigned(dsp_out_accum_results(((f+1)*(32))-1 downto 32*f)));                
                end if;
              end if;
            when others =>
