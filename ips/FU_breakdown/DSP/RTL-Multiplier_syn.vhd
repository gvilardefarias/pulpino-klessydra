...
begin
      ...
      for g in 0 to (ACCL_NUM - FU_NUM) loop
        ...
        if halt_dsp_lat(h) = '0' then
          ...
          for i in 0 to SIMD-1 loop
              ...
              dsp_mul_a(31+32*(i)   + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) <= std_logic_vector(unsigned(dsp_in_mul_operands(15+16*(2*i+1)     + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width) downto  16*(2*i+1) + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width))) * unsigned(dsp_in_mul_operands(15+16*(2*i+1)   + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width) downto  16*(2*i+1) + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width))));
              dsp_mul_b(31+32*(i)   + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) <= std_logic_vector((unsigned(dsp_in_mul_operands(16*(2*i+1) - 1   + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width) downto  16*(2*i) + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width)))   * unsigned(dsp_in_mul_operands(15+16*(2*i+1)   + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width) downto  16*(2*i+1) + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width)))) and unsigned(FUNCT_SELECT_MASK(((h+1)*(32))-1 downto 32*h)));
              dsp_mul_c(31+32*(i)   + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) <= std_logic_vector((unsigned(dsp_in_mul_operands(15+16*(2*i+1)    + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width) downto  16*(2*i+1) + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width))) * unsigned(dsp_in_mul_operands(16*(2*i+1) - 1  + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width) downto  16*(2*i) + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width))))   and unsigned(FUNCT_SELECT_MASK(((h+1)*(32))-1 downto 32*h)));
              dsp_mul_d(31+32*(i)   + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) <= std_logic_vector(unsigned(dsp_in_mul_operands(16*(2*i+1)  - 1   + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width) downto  16*(2*i) + (f)*(2)*(SIMD_Width) + (0)*(SIMD_Width)))   * unsigned(dsp_in_mul_operands(16*(2*i+1) - 1  + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width) downto  16*(2*i) + (f)*(2)*(SIMD_Width) + (1)*(SIMD_Width))));
            end loop;
          end if;
        end if;
      end loop;
  ...
  variable h : integer;
  begin
    mul_tmp_a((((f)+1)*(SIMD)*(Data_Width))-1 downto (SIMD)*(Data_Width)*(f)) <= (others => '0');
    mul_tmp_b((((f)+1)*(SIMD)*(Data_Width))-1 downto (SIMD)*(Data_Width)*(f)) <= (others => '0');
    mul_tmp_c((((f)+1)*(SIMD)*(Data_Width))-1 downto (SIMD)*(Data_Width)*(f)) <= (others => '0');
    mul_tmp_d((((f)+1)*(SIMD)*(Data_Width))-1 downto (SIMD)*(Data_Width)*(f)) <= (others => '0');
    for g in 0 to (ACCL_NUM - FU_NUM) loop
      if multithreaded_accl_en = 1 then
        h := g;  -- set the spm rd/wr ports equal to the "for-loop"
      elsif multithreaded_accl_en = 0 then
      ...
      if mul_en(h) = '1' and (mul_stage_2_en(h) = '1' or recover_state_wires(h) = '1') then
        for i in 0 to SIMD-1 loop
          if MVTYPE_DSP(((h+1)*(2))-1 downto 2*h) /= "10" then
            ------------------------------------------------------------------------------------
            mul_tmp_a(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width)) <= (dsp_mul_a(15+16*(2*i)   + (f)*(SIMD_Width) downto  16*(2*i) + (f)*(SIMD_Width)) & x"0000");
            mul_tmp_d(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width)) <= (x"0000" & dsp_mul_d(15+16*(2*i)   + (f)*(SIMD_Width) downto  16*(2*i) + (f)*(SIMD_Width)));
            ------------------------------------------------------------------------------------
            ...
            mul_tmp_b(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width)) <= (dsp_mul_b(15+16*(2*i)  + (f)*(SIMD_Width) downto  16*(2*i) + (f)*(SIMD_Width)) & x"0000");         -- Modified to only add the partial result to the lower 32-bits   (Ah*Bl)
            mul_tmp_c(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width)) <= (dsp_mul_c(15+16*(2*i)  + (f)*(SIMD_Width) downto  16*(2*i) + (f)*(SIMD_Width)) & x"0000");         -- Modified to only add the partial result to the lower 32-bits   (Al*Bh)
            mul_tmp_d(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width)) <= (dsp_mul_d(31+32*(i)    + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)));                     -- This is the lower 32-bit result of the partial mmultiplication (Al*Bl)
          end if;
        end loop;
      end if;
    end loop;
  ...
  begin
  if rst_ni = '0' then
    dsp_out_mul_results((f+1)*SIMD_Width -1 downto f*SIMD_Width) <= (others => '0');
    elsif rising_edge(clk_i) then
      for g in 0 to (ACCL_NUM - FU_NUM) loop
        if multithreaded_accl_en = 1 then
          h := g;  -- set the spm rd/wr ports equal to the "for-loop"
        ...
        if mul_en(h) = '1' and (mul_stage_2_en(h) = '1' or recover_state_wires(h) = '1') and halt_dsp_lat(h) = '0' then
          for i in 0 to SIMD-1 loop
            dsp_out_mul_results((Data_Width-1)+Data_Width*(i)  + (f)*(SIMD_Width) downto  Data_Width*(i) + (f)*(SIMD_Width))  <= (std_logic_vector(unsigned(mul_tmp_a(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width))) + unsigned(mul_tmp_b(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width))) + unsigned(mul_tmp_c(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width))) + unsigned(mul_tmp_d(((i+1)*(Data_Width))-1 + (f)*(SIMD)*(Data_Width) downto Data_Width*i + (f)*(SIMD)*(Data_Width)))));
          end loop;
        end if;
      end loop;
    end if;
