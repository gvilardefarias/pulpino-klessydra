      ...
      for g in 0 to (ACCL_NUM - FU_NUM) loop
        ...
        if shift_en(h) = '1' and (shifter_stage_1_en(h) = '1' or recover_state_wires(h) = '1') and halt_dsp_lat(h) = '0' then
          for i in 0 to SIMD-1 loop
            dsp_int_shifter_operand(31+32*(i)  + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) <= to_stdlogicvector(to_bitvector(dsp_in_shifter_operand(31+32*(i)  + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width))) srl to_integer(unsigned(dsp_in_shift_amount(((f+1)*(5))-1 downto 5*f))));
          end loop;
          ...
          if MVTYPE_DSP(((h+1)*(2))-1 downto 2*h) = "00" then
            for i in 0 to 4*SIMD-1 loop -- latch the sign bits
              dsp_in_shifter_operand_lat(7+8*i  + (f)*(SIMD_Width) downto  8*i + (f)*(SIMD_Width)) <= (others => dsp_in_shifter_operand((f)*(SIMD_Width) + 7+8*i));
            end loop;
          elsif MVTYPE_DSP(((h+1)*(2))-1 downto 2*h) = "01" then
            for i in 0 to 2*SIMD-1 loop -- latch the sign bits
              for bit_idx in 0 to 15 loop
                dsp_in_shifter_operand_lat((f)*(SIMD_Width) + 16*i + bit_idx) <= dsp_in_shifter_operand((f)*(SIMD_Width) + 15+16*i);
              end loop;
            ...
            for i in 0 to SIMD-1 loop -- latch the sign bits
              for bit_idx in 0 to 31 loop
                dsp_in_shifter_operand_lat((f)*(SIMD_Width) + 32*i + bit_idx) <= dsp_in_shifter_operand((f)*(SIMD_Width) + 31+32*i);
              end loop;
--              dsp_in_shifter_operand_lat(f)(31+32*i downto 32*i) <= (others => dsp_in_shifter_operand(f)(31+32*i));
  ...
  begin
    if rst_ni = '0' then
      dsp_out_shifter_results((f+1)*SIMD_Width -1 downto f*SIMD_Width) <= (others => '0');
    elsif rising_edge(clk_i) then
      for g in 0 to (ACCL_NUM - FU_NUM) loop
        if multithreaded_accl_en = 1 then
          h := g;  -- set the spm rd/wr ports equal to the "for-loop"
        ...
        if shift_en(h) = '1' and (shifter_stage_2_en(h) = '1' or recover_state_wires(h) = '1') and halt_dsp_lat(h) = '0' then
          if    MVTYPE_DSP(((h+1)*(2))-1 downto 2*h) = "10" then
            for i in 0 to SIMD-1 loop
              dsp_out_shifter_results(31+32*(i)  + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) <= dsp_in_shifter_operand_lat_wire(31 +32*(i)  + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width)) or dsp_int_shifter_operand(31+32*(i)  + (f)*(SIMD_Width) downto  32*(i) + (f)*(SIMD_Width));
            end loop;
            ...
            for i in 0 to 2*SIMD-1 loop
              dsp_out_shifter_results(15+16*(i)  + (f)*(SIMD_Width) downto  16*(i) + (f)*(SIMD_Width)) <=  dsp_in_shifter_operand_lat_wire(15 +16*(i)  + (f)*(SIMD_Width) downto  16*(i) + (f)*(SIMD_Width)) or (dsp_int_shifter_operand(15+16*(i)  + (f)*(SIMD_Width) downto  16*(i) + (f)*(SIMD_Width)) and dsp_shift_enabler(15  + (h)*(16) downto  0 + (h)*(16)));
            end loop;
          elsif MVTYPE_DSP(((h+1)*(2))-1 downto 2*h) = "00" then
            for i in 0 to 4*SIMD-1 loop
              dsp_out_shifter_results(7+8*(i)  + (f)*(SIMD_Width) downto  8*(i) + (f)*(SIMD_Width)) <=  dsp_in_shifter_operand_lat_wire(7 +8*(i)  + (f)*(SIMD_Width) downto  8*(i) + (f)*(SIMD_Width)) or  (dsp_int_shifter_operand(7+8*(i)  + (f)*(SIMD_Width) downto  8*(i) + (f)*(SIMD_Width)) and dsp_shift_enabler(7  + (h)*(16) downto  0 + (h)*(16)));
            end loop;
          end if;
        end if;
      end loop;
  ...
  variable h : integer;
  begin
    dsp_in_shifter_operand_lat_wire(((f+1)*(SIMD_Width))-1 downto SIMD_Width*f) <= (others => '0');
    for g in 0 to (ACCL_NUM - FU_NUM) loop
      if multithreaded_accl_en = 1 then
        h := g;  -- set the spm rd/wr ports equal to the "for-loop"
      elsif multithreaded_accl_en = 0 then
      ...
      dsp_shift_enabler(((h+1)*(16))-1 downto 16*h) <= (others => '0');
        ...
        if MVTYPE_DSP(((h+1)*(2))-1 downto 2*h) = "01" then
          for idx in 0 to 15 loop
            if idx <= (15 - to_integer(unsigned(dsp_in_shift_amount(3  + (h)*(5) downto  0 + (h)*(5))))) then
              dsp_shift_enabler((h)*(16) + idx) <= '1';
            else
              dsp_shift_enabler((h)*(16) + idx) <= '0';
            end if;
          end loop;
          ...
          for i in 0 to SIMD-1 loop
            ...
            for idx in 0 to 31 loop
              if idx = (31 - to_integer(unsigned(dsp_in_shift_amount(4  + (f)*(5) downto  0 + (f)*(5))))) then
                dsp_in_shifter_operand_lat_wire(31+32*(i)  + (f)*(SIMD_Width) downto idx +32*(i) + (f)*(SIMD_Width)) <= 
          ...
          for i in 0 to 2*SIMD-1 loop
            ...
            for idx in 0 to 15 loop
              if idx = (15 - to_integer(unsigned(dsp_in_shift_amount(3  + (f)*(5) downto  0 + (f)*(5))))) then
                dsp_in_shifter_operand_lat_wire(15+16*(i)  + (f)*(SIMD_Width) downto idx +16*(i) + (f)*(SIMD_Width)) <= 
          ...
          for i in 0 to 4*SIMD-1 loop
            ...
            for idx in 0 to 7 loop
              if idx = (7 - to_integer(unsigned(dsp_in_shift_amount(2  + (f)*(5) downto  0 + (f)*(5))))) then
                dsp_in_shifter_operand_lat_wire(7+8*(i)  + (f)*(SIMD_Width) downto idx +8*(i) + (f)*(SIMD_Width)) <= 
